...
function createServiceAPI(serviceName) {
  ...
  return function(user_options) {
    ...
    try {
      ...
      console.log("index: service path to add " + servicePath);
      ...
      if (e.code === 'MODULE_NOT_FOUND') {
        ...
        throw new Error('ERROR: ' + util.format(msg, serviceName, version) + ' service path is ' + servicePath);
      } else {
        throw e;
      }
    }
...
var watson = {
   ...
   * @function
   * @param {{}} options
   * @returns  {ToneConversation}
   */
  tone_conversation: createServiceAPI('tone_conversation')
};
